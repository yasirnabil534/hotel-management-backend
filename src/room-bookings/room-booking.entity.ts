...
export class RoomBookingEntity {
  ...
  @ApiProperty({ example: 750.00, description: 'Subtotal (price × nights)' })
  subtotal: number;

  @ApiProperty({ example: 50.00, description: 'Discount amount' })
  discount: number;

  @ApiProperty({ example: 700.00, description: 'Total amount (subtotal - discount)' })
  total: number;

  @ApiProperty({ example: 200.00, description: 'Amount paid' })
  paid: number;

  @ApiProperty({ example: 500.00, description: 'Balance remaining (total - paid)' })
  balance: number;
